...
static VkFormat chooseFormat(VkPhysicalDevice phy,
                             ...
                             const std::array<VkFormat, N> &desired_formats)
{
    for (auto fmt : desired_formats) {
        ...
        std::cerr << "Warning: preferred format not available" << std::endl;
    }

    FATAL("Unable to find required features in given formats");
}

static std::pair<VkBuffer, VkMemoryRequirements> makeUnboundBuffer(
    const Device &dev,
    VkDeviceSize num_bytes,
...
static MemoryTypeIndices findTypeIndices(const Device &dev,
    const Backend &backend,
    const std::array<VkFormat, size_t(TextureFormat::COUNT)> &tex_formats)
{
...
{
    static std::array<uint32_t, size_t(TextureFormat::COUNT)> fmt_sizes {
        4,
        4,
        2,
        1,
...
static std::array<VkFormat, size_t(TextureFormat::COUNT)> chooseTextureFormats(
    const Device &dev, const Backend &backend)
{
    std::array<VkFormat, size_t(TextureFormat::COUNT)> fmts;
    ...
    setFormat(TextureFormat::R8G8B8A8_SRGB, std::array {
        VK_FORMAT_R8G8B8A8_SRGB,
    });

    setFormat(TextureFormat::R8G8B8A8_UNORM, std::array {
        VK_FORMAT_R8G8B8A8_UNORM,
    });

    setFormat(TextureFormat::R8G8_UNORM, std::array {
        VK_FORMAT_R8G8_UNORM,
        VK_FORMAT_R8G8B8A8_UNORM,
    });

    setFormat(TextureFormat::R8_UNORM, std::array {
        VK_FORMAT_R8_UNORM,
        VK_FORMAT_R8G8B8A8_UNORM,
    });

    setFormat(TextureFormat::R32G32B32A32_SFLOAT, std::array {
        VK_FORMAT_R32G32B32A32_SFLOAT,
    });

    setFormat(TextureFormat::R32_SFLOAT, std::array {
        VK_FORMAT_R32_SFLOAT,
    });

    setFormat(TextureFormat::BC5_UNORM, std::array {
        VK_FORMAT_BC5_UNORM_BLOCK,
    });

    setFormat(TextureFormat::BC7_UNORM, std::array {
        VK_FORMAT_BC7_UNORM_BLOCK,
    });

    setFormat(TextureFormat::BC7_SRGB, std::array {
...
std::optional<SparseBuffer> MemoryAllocator::makeSparseBuffer(
    VkDeviceSize num_bytes,
    bool dev_addr)
{
...
std::optional<LocalBuffer> MemoryAllocator::makeLocalBuffer(
    VkDeviceSize num_bytes,
    VkBufferUsageFlags usage,
    bool dev_addr)
{
...
std::optional<LocalBuffer> MemoryAllocator::makeLocalBuffer(VkDeviceSize num_bytes,
                                                       bool dev_addr)
{
    return makeLocalBuffer(num_bytes, local_buffer_usage_flags_, dev_addr);
}

std::optional<LocalBuffer> MemoryAllocator::makeIndirectBuffer(
    VkDeviceSize num_bytes)
{
...
std::pair<LocalTexture, TextureRequirements> MemoryAllocator::makeTexture1D(
    uint32_t width,
    uint32_t mip_levels,
    VkFormat fmt)
{
...
std::pair<LocalTexture, TextureRequirements> MemoryAllocator::makeTexture2D(
    uint32_t width,
    uint32_t height,
    uint32_t mip_levels,
    VkFormat fmt)
...
std::pair<LocalTexture, TextureRequirements> MemoryAllocator::makeTexture3D(
    uint32_t width,
    uint32_t height,
    uint32_t depth,
    uint32_t mip_levels,
...
template <int dims>
std::pair<LocalTexture, TextureRequirements> MemoryAllocator::makeTexture(
    uint32_t width,
    uint32_t height,
    uint32_t depth,
    uint32_t mip_levels,
...
std::optional<VkDeviceMemory> MemoryAllocator::alloc(VkDeviceSize num_bytes)
{
    VkMemoryAllocateInfo alloc;
    alloc.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    alloc.pNext = nullptr;
    ...
    if (res == VK_ERROR_OUT_OF_DEVICE_MEMORY) {
        return std::optional<VkDeviceMemory>();
    }
