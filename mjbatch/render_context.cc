...
struct Backend::Init {
    ...
    static inline Backend::Init init(PFN_vkGetInstanceProcAddr get_inst_addr,
                                     ...
                                     std::span<const char *const> extra_exts);
};
...
static bool checkValidationAvailable(const InitializationDispatch &dt)
{
    ...
    std::vector<VkExtensionProperties> exts(num_exts);
    ...
    std::span<const char *const> extra_exts)
{
    ...
    app_info.pApplicationName = "mjbatch";
    app_info.pEngineName = "mjbatch";
    app_info.apiVersion = VK_API_VERSION_1_2;

    std::vector<const char *> layers;
    std::vector<const char *> extensions(extra_exts.size());

    for (const char *extra_ext : extra_exts) {
        extensions.push_back(extra_ext);
    }

    std::vector<VkValidationFeatureEnableEXT> val_enabled;
    ...
    const DeviceID &dev_id) const
{
    ...
    std::vector<VkPhysicalDevice> phys(num_gpus);
...
static QueueFamilyChoices chooseQueueFamilies(
    ...
    std::span<const VkSurfaceKHR> present_surfaces)
{
    ...
    std::span<const VkSurfaceKHR> present_surfaces)
{
    ...
    std::vector<VkQueueFamilyProperties2> queue_family_props(num_queue_families);
    ...
    const uint32_t num_gfx_queues =
        std::min(desired_gfx_queues, qf_choices.numGFXQueues);
    const uint32_t num_compute_queues =
        std::min(desired_compute_queues, qf_choices.numComputeQueues);
    const uint32_t num_transfer_queues =
        std::min(desired_transfer_queues, qf_choices.numTransferQueues);

    std::array<VkDeviceQueueCreateInfo, 3> queue_infos {};
    std::vector<float> gfx_pris(num_gfx_queues, VulkanConfig::gfx_priority);
    std::vector<float> compute_pris(num_compute_queues,
                               VulkanConfig::compute_priority);
    std::vector<float> transfer_pris(num_transfer_queues,
