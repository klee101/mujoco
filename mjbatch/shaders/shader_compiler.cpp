...
{
    void *lib = dlopen("libdxcompiler.so", RTLD_NOW | RTLD_NOLOAD | RTLD_NODELETE);
    assert(lib != nullptr);
    dlclose(lib);
}
...
#define REQ_DXC(expr, msg) \
    ::mujoco::mjbatch::checkDXC((expr), msg, __FILE__, __LINE__,\
                                COMPILER_FUNCTION_NAME)
...
static std::vector<wchar_t> toWide(const char *str)
{
    std::vector<wchar_t> out(strlen(str) + 1);
    size_t num_conv = mbstowcs(out.data(), str, out.size());
    if ((int64_t)num_conv != out.size() - 1) {
...
{
    std::vector<wchar_t> lshader_path = toWide(shader_path);
...
static std::vector<uint32_t> hlslToSPV(
    ...
    std::span<const ShaderCompiler::MacroDefn> macro_defns,
...
#undef TYPE_STR
    ...
    std::optional<std::vector<wchar_t>> wentry = std::nullopt;
    if (entry_func != nullptr) {
    ...
    std::vector<std::vector<wchar_t>> wdefines(macro_defns.size());
    ...
    std::vector<std::vector<wchar_t>> wincs(include_dirs.size());

    for (int64_t i = 0; i < include_dirs.size(); i++) {
    ...
    std::vector<uint32_t> bytecode(num_shader_bytes / 4);
    ...
    CountT num_spv_bytes)
{
    ...
    std::vector<EntryPoint> entry_points(rfl_mod.entry_point_count);
    std::vector<std::vector<Binding>> tmp_desc_sets(0);
    for (CountT i = 0; i < entry_points.size(); i++) {
        ...
        for (uint32_t j = 0; j < spv_rfl_entry.descriptor_set_count; j++) {
            ...
            if (spv_desc_set.set >= tmp_desc_sets.size()) {
                tmp_desc_sets.resize(spv_desc_set.set + 1);
            }
    ...
    std::vector<Binding> bindings_out(total_num_bindings);
    std::vector<DescriptorSet> desc_sets_out(num_sets);
